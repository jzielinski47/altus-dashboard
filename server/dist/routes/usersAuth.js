...
router.post(
  ...
  (0, express_validator_1.checkSchema)(
    validationSchemas_1.signupValidationSchema
  ),
  (req, res) => {
    ...
    if (!result.isEmpty()) {
      ...
      const { username, email, password } = (0,
      express_validator_1.matchedData)(req);
      // at this point make sure the password is cyphered
      const usernameExists = constans_1.usersCollection.find(
      ...
      if (usernameExists) {
        ...
        const newRecord = {
          id:
            constans_1.usersCollection[constans_1.usersCollection.length - 1]
              .id + 1,
          username,
          password,
          email,
...
router.post("/api/auth", (req, res) => {
  ...
  const findUser = constans_1.usersCollection.find(
    (user) => user.username === username
  );
  if (!findUser) {
